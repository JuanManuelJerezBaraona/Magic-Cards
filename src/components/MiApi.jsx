...
function MiApi({ setInfo, Busqueda, setBusqueda }) { // Define el componente funcional MiApi y recibe las propiedades 'setInfo' y 'setBusqueda'.
    
    useEffect(() => { // Usa useEffect para realizar efectos secundarios cuando el componente se monta.
    ...
    const consultarApi = async () => { // Define una función asincrónica llamada consultarApi.
        ...
        const cartasOrdenadas = data.cards.slice().sort((a, b) => { // Ordena las cartas alfabéticamente por nombre.
    ...
    return (
        <> {/* Abre un fragmento React para envolver varios elementos sin un div raíz. */}
            <div className="d-flex flex-wrap justify-content-center gap-5 mb-5">
                {Busqueda.map((tarjeta, index) => ( // Mapea y muestra cada tarjeta en 'Busqueda'.
                    tarjeta.imageUrl && ( // Si hay una URL de imagen, muestra la tarjeta.
                        <Card key={index} style={{ width: '16rem' }} className="shadow-lg zoom transition">
                            <Card.Img variant="top" src={tarjeta.imageUrl} alt={tarjeta.name} className="card-image" />
                            <Card.Body>
                                <Card.Title className="mb-3">{tarjeta.name}</Card.Title>
                                <Card.Text><strong>Rarity:</strong> {tarjeta.rarity}.</Card.Text>
                                <Card.Text><strong>Artist:</strong> {tarjeta.artist}.</Card.Text>
                            </Card.Body>
                        </Card>
                    )
                ))}
